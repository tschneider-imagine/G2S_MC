...
func TestDashboardRouteServesHTML(t *testing.T) {
	...
	if !strings.Contains(rr.Body.String(), "Use Selected as Test EGMs") {
	...
	if !strings.Contains(rr.Body.String(), "Cabinet Signal Timeline") {
		t.Fatalf("expected cabinet signal timeline panel")
	}
	...
	if !strings.Contains(rr.Body.String(), "id=\"operator-drill-form\"") {
		t.Fatalf("expected traffic simulator form")
	}
	...
	if !strings.Contains(rr.Body.String(), "id=\"heartbeat-policy-block-after-missed\"") {
		t.Fatalf("expected heartbeat policy missed-threshold input")
	}
	...
	if !strings.Contains(rr.Body.String(), "id=\"session-package-export-button\"") {
		t.Fatalf("expected controller package export action")
	}
	if !strings.Contains(rr.Body.String(), "id=\"session-package-export-message\"") {
		t.Fatalf("expected controller package export message marker")
	}
